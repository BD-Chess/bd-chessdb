...
| `BD_trading_2026.html` | Trading research/evidence page | Full historic body re-encrypted as one 8Z block | Active after Trading unlock | Yes |
| `BD_HSR_PiX_v1_3c_FINAL_ANALYSIS_20260621.html` | HSR/PiX analysis | Full historic body re-encrypted as one 8Z block | Active after Trading unlock | Yes |
| `BDTS.html` | Dual-book research protocol | Full historic body re-encrypted as one 8Z block | Active after Trading unlock | Yes |
| `BBSH.html` | BB Surfing Hedge protocol/calculator | Full historic body re-encrypted as one 8Z block | Active after Trading unlock | Yes |
| `BD_HSR_PiX_v1_3c_SOL_MANUAL_PROTOCOLS.html` | HSR/PiX protocols | Full historic body re-encrypted as one 8Z block | Active after Trading unlock | Yes |
| `BD_Hedge_Separation_PiX_Strategy_Explainer.html` | HSR/PiX explainer | Full historic body re-encrypted as one 8Z block | Active after Trading unlock | Yes |
| `SM_trader.html` | Public-data / virtual-money browser paper interface | Full historic body re-encrypted as one 8Z block | **Engine migration hold** | Yes, as protected archive |
| `ZZ_trader.html` | Public-feed / virtual-money browser paper interface | Full historic body re-encrypted as one 8Z block | **Engine migration hold** | Yes, as protected archive |
| `DCC_trader.html` | Virtual-balance browser research interface | Full historic body re-encrypted as one 8Z block | **Engine migration hold** | Yes, as protected archive |
...
- Page-specific `.gitattributes` entries mark the re-encrypted historic pages
  as non-text diffs, so the PR review cannot re-render their predecessor
  plaintext while showing a ciphertext replacement. The HTML itself remains
  available for an authorized source review.
