 ...
 *
 * Build 2026-03-25 · AIF8 🌱\n * Patch 2026-03-25 · CSV-safe exports + winner fields
 */
"use strict";
...
function predictedWinnerFromAcc(wAcc, bAcc) {
  if (wAcc > bAcc) return 'w';
  if (bAcc > wAcc) return 'b';
  return '=';
}
...
function analyzeGame(parsed, depth, topN, floor) {
  ...
  const predictedWinner = predictedWinnerFromAcc(wPct, bPct);
  const actualWinner = actualWinnerFromResult(headers.Result || '*');
  const predictionCorrect = actualWinner === '?' ? null : predictedWinner === actualWinner;
...
async function modeAnalyze(pgnPath) {
  ...
  let csv = 'file,white,black,result,dcc_depth,w_acc,b_acc,gap,gap_cat,predicted_winner,actual_winner,prediction_correct,w_stab,b_stab,w_collapses,b_collapses,w_tunnels,b_tunnels,w_coverage,b_coverage,turning_point_ply,w_sustained,w_building,w_spike,w_collapse,b_sustained,b_building,b_spike,b_collapse,total_moves\n';
  for (const r of allResults) {
    csv += csvRow([
      ...
      r.summary.predicted_winner, r.summary.actual_winner,
      r.summary.prediction_correct === null ? '' : (r.summary.prediction_correct ? 1 : 0),
...
async function modeBatch(dirPath) {
  ...
  let csv = 'file,white,black,result,w_accuracy,b_accuracy,gap,predicted_winner,actual_winner,prediction_correct,collapses_w,collapses_b,avg_stab\n';
...
async function modeTournament() {
  ...
  for (let gn = 0; gn < numGames; gn++) {
    ...
    tournamentCSV.push(csvRow([gn + 1, dccColor, result, winner, history.length, dccAcc, rawAcc, dccCol, rawCol]).trimEnd());
