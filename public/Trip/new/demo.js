...
const $=id=>document.getElementById(id);
  ...
  function renderDemo() {
    ...
    const update = () => {
      ...
      $('factorialResult').innerHTML = `<strong>(${n}−1)! = ${total.toLocaleString('en-US')}</strong><p>${bi('Estimated full search','Ocenjen čas celotnega iskanja')}: <b>${window.TripUI.t(TripBruteForce.duration(sec))}</b></p><p>${ratio.toExponential(2)} × ${bi('the age of the universe','starost vesolja')}</p>`;
