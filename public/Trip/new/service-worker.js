/* Offline shell for the public Trip LAB only. Bump CACHE with every LAB asset change.
   No API, road response, or encrypted payload is persisted. */
const CACHE = '8z-trip-lab-public-20260924-1';
...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    ...
    await Promise.all(keys.filter(key => key.startsWith('8z-trip-lab-public-') && key !== CACHE)
