...
(() => {
  ...
  async function start(job,tspData){
    ...
    const reply=await rpc({type:'start',runId:ctx.id,problemHash:ctx.problemHash,problem:ctx.problem,options:{engine:'bd-trip-browser-r1.1.0',seed:42,workerCount:Number($('mdlWorkers').value)},ui,visible:!document.hidden,startRequested,clientSessionId,diagnosticsEnabled:$('diagnosticsEnabled')?.checked!==false,environment:{userAgent:navigator.userAgent,platform:navigator.platform||null,hardwareConcurrency:navigator.hardwareConcurrency||null,deviceMemory:navigator.deviceMemory||null}});accept(reply.receipt,ctx);
