...
(() => {
  'use strict';
  const $=id=>document.getElementById(id),ENGINE='bd-trip-browser-r1.1.0';
  ...
  function detach(){creation++;cancelCreation=false;connection=null;terminate();}
  ...
  function rpc(message,timeout=20000){
    ...
    const requestId=crypto.randomUUID(),packet={v:1,requestId,...message};
  ...
  async function ensureWorker(){
    ...
    try{
      ...
      worker.onerror=()=>{detach();hooks.disconnected?.(Error(say('Experimental Worker failed. Reload and recover.','Eksperimentalni Worker ni uspel. Znova odpri stran in obnovi stanje.')));};
      await rpc({type:'init',credential:credential.slice()});
    }catch(e){terminate();throw e;}finally{if(workerURL){URL.revokeObjectURL(workerURL);workerURL=null;}}
  }
  ...
  async function unlock(event){
    ...
    try{
      ...
      const answer=await response.json();if(!response.ok||answer.schema!=='TripUnlockV1'||answer.engine!==ENGINE)throw Error('unlock');
      const next=TripShield.bytes(answer.credential);if(next.length!==32)throw Error('key');
      const resource=await fetch('mdl-worker.enc.json?v=20260914-mdl-browser1',{cache:'no-store',signal:AbortSignal.timeout(20000)});if(!resource.ok)throw Error('payload');
  ...
  function accept(data,ctx){
    ...
    hooks.message?.({data:{type:terminal?'result':'progress',jobId:ctx.localId,algorithm:'deep',private:true,phase:data.state,reason:data.state==='completed_optimum'?'optimum':data.state==='paused_budget'?'budget':terminal?'cancelled':null,cancelled:terminal&&!data.optimumReached&&data.state!=='paused_budget',exact:data.optimumReached,canContinue:data.canContinue,
    ...
    hint(data.storageError?say('Local save failed; keep this tab open.','Lokalna hramba ni uspela; ohrani ta zavihek odprt.'):data.persisted?say('Encrypted checkpoint saved on this device.','Šifrirano stanje je shranjeno na tej napravi.'):say('Calculating locally.','Računam lokalno.'));
  }
  async function start(job,tspData){
    ...
    const reply=await rpc({type:'start',runId:ctx.id,problemHash:ctx.problemHash,problem:ctx.problem,options:{engine:ENGINE,seed:42},ui});accept(reply.receipt,ctx);
    if(cancelCreation)await cancel();else if(document.hidden)await visibility();
  }
  async function operation(type,extra={}){const ctx=connection;if(!ctx)throw Error('Recover the local run first');const reply=await rpc({type,runId:ctx.id,problemHash:ctx.problemHash,engine:ENGINE,controlSeq:ctx.controlSeq+1,...extra});if(reply.receipt)accept(reply.receipt,ctx);return reply;}
  ...
  async function recover(){if(busy)return;await ensureWorker();const reply=await rpc({type:'restore'});if(reply.type==='empty'){hint(say('No saved checkpoint on this device.','Na tej napravi ni shranjenega stanja.'));return;}
    const saved=reply.restored,job=await hooks.recover(saved.ui),ctx=await makeContext(job,{entry:saved.ui.entry});if(ctx.problemHash!==reply.receipt.problemHash)throw Error('Saved input identity mismatch');ctx.id=reply.receipt.jobId;connection=ctx;accept(reply.receipt,ctx);
  }
