(() => {
  ...
  const HELP_HTML = `<h2>📖 Guide</h2><hr><p><strong>1. Input:</strong> Type cities or GPS coords.</p><p><strong>2. Optimize:</strong> 'Standard' is fast. 'Deep' is for complex loops.</p><p><strong>3. AI:</strong> Ask Gemini to "Add stops in Italy".</p>`;
  const ABOUT_HTML = `<h2>ℹ️ About</h2><hr><div style="background:rgba(59,130,246,0.1);padding:10px;border-radius:6px;border-left:3px solid #3b82f6;"><h3>The Morocco Story</h3><p>Built during a backpacking trip to solve the "Traveler's Salesman Problem" that normal maps can't handle.</p></div><p>Version 2026.1</p>`;
  ...
  function updateModeButtons() {
    $('btnDriving').classList.toggle('active', currentTravelMode === 'DRIVING');
    $('btnWalking').classList.toggle('active', currentTravelMode === 'WALKING');
  }
  function toggleChat(open) {
    const p = $('chatPanel');
    if(open) { p.classList.add('open'); $('chatInput').focus(); } else { p.classList.remove('open'); }
  }
    ...
    if ($('chkDirect').checked) {
      ...
      for(let i=0; i<path.length-1; i+=24) {
        ...
        directionsService.route({
          origin: seg[0], destination: seg[seg.length-1], waypoints: seg.slice(1,-1).map(l=>({location:l, stopover:true})),
  ...
  worker.onmessage = (e) => {
    ...
    // 2. FIX BUTTONS (Ask AI + Maps)
    const linksEl = $('links');
    linksEl.innerHTML = ''; // Clear

    // "Ask AI" Button
    const btnAI = document.createElement('button');
    btnAI.className = 'btn-secondary';
    btnAI.style.marginBottom = '10px';
    btnAI.innerHTML = '✨ Ask AI: Is this logical?';
    btnAI.onclick = () => {
        toggleChat(true);
        const prompt = `Review this route logic:\n${pointsSorted.map(p=>p.name).join(' -> ')}`;
        $('chatInput').value = prompt;
    };
    linksEl.appendChild(btnAI);
    ...
    const url = `${baseUrl}&origin=${origin}&destination=${dest}&waypoints=${waypoints}&travelmode=${currentTravelMode.toLowerCase()}`;
    
    const btnMap = document.createElement('a');
    btnMap.className = 'btn-primary';
    btnMap.style.display = 'flex'; 
    btnMap.style.justifyContent = 'center';
    btnMap.style.textDecoration = 'none';
    btnMap.target = '_blank';
    btnMap.href = url;
    btnMap.textContent = 'Open in Google Maps ↗';
    linksEl.appendChild(btnMap);
  };
  ...
  async function initAI() {
    try {
      ...
      if(s) { s.innerHTML = ''; v.forEach(m => { const o = document.createElement('option'); o.value = m.name; o.textContent = m.displayName; s.appendChild(o); }); currentGeminiModel = v[0]?.name; s.onchange = () => currentGeminiModel = s.value; }
    } catch(e){}
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('btnSendChat').onclick = async () => {
      ...
      i.value = ''; $('chatHistory').innerHTML += `<div class="msg user">${t}</div>`;
      const r = await callAI(t);
      const m = r.match(/\{ADD:\s*(.*?)\}/g); if(m) m.forEach(x => { const loc = x.replace(/\{ADD:\s*|\}/g,'').trim(); if(!$('input').value.includes(loc)) $('input').value += '\n' + loc; });
      $('chatHistory').innerHTML += `<div class="msg ai"><strong>Gemini:</strong> ${r.replace(/\n/g,'<br>')}</div>`; $('chatHistory').scrollTop = $('chatHistory').scrollHeight;
    };
    ...
    $('tripSearch').oninput = (e) => {
      ...
      document.querySelectorAll('.tree-item').forEach(i => {
        ...
        if(q && i.style.display==='block') { let p = i.parentElement; while(p && p.id!=='presetTree') { if(p.classList.contains('tree-group')) p.classList.add('open'); p=p.parentElement; }}
      });
    };
  });
})();