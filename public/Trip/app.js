  ...
  function initTripTree() {
    ...
    window.TRIP_LIBRARY.forEach(region => {
      ...
      region.categories.forEach(cat => {
        ...
        cHeader.addEventListener('click', (e) => { e.stopPropagation(); cGroup.classList.toggle('open'); cHeader.querySelector('.tree-icon').textContent = cGroup.classList.contains('open') ? '[-]' : '[+]'; });
