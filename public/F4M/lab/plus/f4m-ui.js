...
const VERSION='2.1.0',PREF='flip4m.lab.2.1.preferences';
...
function showStats(bundle){
 ...
 $('searchInfo').textContent=t('workLabel')+' '+r.work.toLocaleString()+' · '+(r.safetyComplete?t('safe'):t('incomplete'))+' · '+t('analysisNote');
...
function request(kind='ai'){
 ...
 const options=inSimulation&&kind==='ai'?batch.config.budget:{mode:'time',work:300000,...(levels[prefs.difficulty]||levels.casual)};
 const dcc=inSimulation&&kind==='ai'?batch.config.dcc:dccConfig(),started=now();render();
 searchClock=setInterval(()=>{if(id!==job)return;$('statTime').textContent=((now()-started)/1000).toFixed(1)+' s';if(options.mode==='time')$('searchBar').style.width=Math.min(95,(now()-started)/options.ms*100)+'%';},100);
 function receive(data){
  if(data.id!==job||E.key(state())!==key)return;
  if(data.type==='progress'){const r=data.report;$('statDepth').textContent=r.depth;$('statNodes').textContent=r.nodes.toLocaleString();if(options.mode==='work')$('searchBar').style.width=Math.min(95,r.work/options.work*100)+'%';return;}
  ...
  const bundle=data.answer;lastStats=bundle;showStats(bundle);$('searchBar').style.width='100%';
 ...
 try{
  ...
  else worker=new Worker('f4m-worker.js?v=2.1.0');worker.onmessage=e=>receive(e.data);worker.onerror=e=>receive({id,type:'error',error:e.message});worker.postMessage({id,state:E.copy(state()),options,dcc});}
...
function startSim(){
 ...
 const length=$('simLength').value,cfg={pairs:length==='single'?1:Number(length),single:length==='single',seed:Number($('simSeed').value),tools:Number($('simTools').value),start:$('simStartAt').value,red:$('simRed').value,yellow:$('simYellow').value,
  budget:{mode:$('computeMode').value,work:Number($('simWork').value),ms:Number($('simMs').value),depth:12},dcc:dccConfig()};
...
window.F4MLab=Object.freeze({version:VERSION,snapshot:()=>JSON.parse(JSON.stringify(envelope())),status:()=>({paused,thinking,animating,loading,inSimulation,cursor,job}),validateSave:validateSession,flush:()=>Store.flush()});
})();
