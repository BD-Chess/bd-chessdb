...
# ─────────────────────────────────────────────────────────────
#  Phase 1 — Strategy-specific solvers
# ─────────────────────────────────────────────────────────────

STRATEGIES = ["random", "mrv", "naked", "full", "human"]

def solve_with_strategy(grid_input, strategy, size=9):
    """
    Solve puzzle with a specific strategy. Returns (solved_grid, log).
    Strategies: random, mrv, naked, full, human.
    All produce identical log format for metric computation.
    """
    if strategy == "full":
        return solve_logged(grid_input, size)  # current solver

    grid = deepcopy(grid_input)
    log = []
    step = [0]
    MAX_STEPS = 10000  # safety limit
    MAX_TIME = 30.0    # seconds per solve
    _t0 = time.time()

    def add_log(r, c, v, strat, cb, ca, cascade, bt):
        step[0] += 1
        log.append({"step": step[0], "row": r, "col": c, "value": v,
                     "strategy": strat, "candidates_before": cb,
                     "candidates_after": ca, "cascade_depth": cascade,
                     "backtrack": bt})

    def find_naked(g):
        cands = get_candidates(g, size)
        for (r, c), vals in cands.items():
            if len(vals) == 1:
                return r, c, next(iter(vals)), "naked_single"
        return None
    ...
    def find_hidden(g):
        ...
        for r in range(size):
            for v in range(1, size + 1):
                if any(g[r][c] == v for c in range(size)):
                    continue
                possible = [(rr, cc) for (rr, cc), vs in cands.items() if rr == r and v in vs]
                if len(possible) == 1:
                    return possible[0][0], possible[0][1], v, "hidden_single"
        # Cols
        for c in range(size):
            for v in range(1, size + 1):
                if any(g[r][c] == v for r in range(size)):
                    continue
                possible = [(rr, cc) for (rr, cc), vs in cands.items() if cc == c and v in vs]
                if len(possible) == 1:
                    return possible[0][0], possible[0][1], v, "hidden_single"
        # Boxes
        for br in range(0, size, box_size):
            for bc in range(0, size, box_size):
                for v in range(1, size + 1):
                    ...
                    if present:
                        continue
                    possible = [(rr, cc) for (rr, cc), vs in cands.items()
                                if br <= rr < br + box_size and bc <= cc < bc + box_size and v in vs]
                    if len(possible) == 1:
                        return possible[0][0], possible[0][1], v, "hidden_single"
        return None

    def find_pointing_pairs(g):
        """Pointing pairs: if a value in a box is confined to one row/col, eliminate from rest of that row/col."""
        cands = get_candidates(g, size)
        box_size = int(math.isqrt(size))
        # This is a constraint reduction, not a placement — for Phase 1 we treat it
        # as finding cells that become naked singles AFTER pointing pair elimination
        # Simplified: find naked singles that emerge after constraint tightening
        for br in range(0, size, box_size):
            for bc in range(0, size, box_size):
                box_cells = [(r, c) for r in range(br, br + box_size) for c in range(bc, bc + box_size)]
                for v in range(1, size + 1):
                    cells_with_v = [(r, c) for r, c in box_cells if (r, c) in cands and v in cands[(r, c)]]
                    if len(cells_with_v) < 2:
                        continue
                    rows = set(r for r, c in cells_with_v)
                    cols = set(c for r, c in cells_with_v)
                    if len(rows) == 1:
                        # All in same row — v can be eliminated from rest of row outside box
                        row = next(iter(rows))
                        for c in range(size):
                            if (bc <= c < bc + box_size):
                                continue
                            if (row, c) in cands and v in cands[(row, c)] and len(cands[(row, c)]) == 2:
                                # Removing v would leave naked single
                                remaining = cands[(row, c)] - {v}
                                if len(remaining) == 1:
                                    return row, c, next(iter(remaining)), "pointing_pair"
                    if len(cols) == 1:
                        col = next(iter(cols))
                        for r in range(size):
                            if (br <= r < br + box_size):
                                continue
                            if (r, col) in cands and v in cands[(r, col)] and len(cands[(r, col)]) == 2:
                                remaining = cands[(r, col)] - {v}
                                if len(remaining) == 1:
                                    return r, col, next(iter(remaining)), "pointing_pair"
        return None

    def propagate(g, techniques):
        """Propagate using enabled techniques. Returns list of (r,c,v,strategy)."""
        placed = []
        progress = True
        while progress:
            progress = False
            if "naked" in techniques:
                found = find_naked(g)
                if found:
                    r, c, v, s = found
                    g[r][c] = v
                    placed.append((r, c, v, s))
                    progress = True
                    continue
            if "hidden" in techniques:
                ...
                if found:
                    ...
                    if g[r][c] == 0:
                        ...
                        progress = True
                        continue
            if "pointing" in techniques:
                found = find_pointing_pairs(g)
                if found:
                    r, c, v, s = found
                    if g[r][c] == 0:
                        g[r][c] = v
    ...
    # Strategy technique sets
    technique_map = {
        "random": [],
        "mrv": [],
        "naked": ["naked"],
        "human": ["naked", "hidden", "pointing"],
    }
    techniques = technique_map.get(strategy, [])

    def solve_rec(g):
        if step[0] >= MAX_STEPS or (time.time() - _t0) > MAX_TIME:
            return False  # bail out
        # Propagation phase
        cb = count_total_candidates(g, size)
        placed = propagate(g, techniques)
        cascade_len = len(placed)
        for i, (r, c, v, s) in enumerate(placed):
            ca = count_total_candidates(g, size)
            add_log(r, c, v, s, cb, ca, cascade_len - 1 if i == 0 else 0, False)
            cb = ca

        # Check solved
        empty = [(r, c) for r in range(size) for c in range(size) if g[r][c] == 0]
        if not empty:
            return True

        # Check contradiction
        cands = get_candidates(g, size)
        if any(len(v) == 0 for v in cands.values()):
            for r, c, v, _ in reversed(placed):
                g[r][c] = 0
            return False
        ...
        if strategy == "random":
            ...
            cell = _rnd.choice(empty)
            r, c = cell
            vals = list(cands.get((r, c), set()))
            if not vals:
                for r2, c2, v2, _ in reversed(placed):
                    g[r2][c2] = 0
                return False
            _rnd.shuffle(vals)
        else:
            # MRV for all other strategies
            r, c = min(cands.keys(), key=lambda k: len(cands[k]))
            vals = sorted(cands[(r, c)])
        ...
        for v in vals:
            ...
            g[r][c] = v
            ca = count_total_candidates(g, size)
            add_log(r, c, v, "guess", cb, ca, 0, False)
            ...
            for rr in range(size):
                for cc in range(size):
                    g[rr][cc] = saved[rr][cc]
            g[r][c] = 0
            ca2 = count_total_candidates(g, size)
            add_log(r, c, v, "backtrack", ca, ca2, 0, True)
...
# ─────────────────────────────────────────────────────────────
#  Phase 1 — Same puzzle × 5 strategies
# ─────────────────────────────────────────────────────────────

PHASE1_CSV_FIELDS = [
    "puzzle_id", "size", "difficulty", "difficulty_num", "n_givens", "strategy",
    "n_steps", "n_backtracks", "n_guesses", "max_cascade",
    "LZ_process", "LZ_solution", "LZ_cascade",
    "ADSR_bimodality", "ADSR_selfcal",
    "LZ_multi_2", "LZ_multi_3", "LZ_multi_4", "LZ_multi_5", "LZ_multi_8", "best_k",
...
STRATEGY_RANK = {"random": 1, "mrv": 2, "naked": 3, "full": 4, "human": 5}


def run_phase1(count=200, size=9, seed=42):
    """Generate puzzles and solve each with all 5 strategies."""
    random.seed(seed)
    results = []

    # Balanced distribution
    if count >= 40:
        dist = {"easy": int(count * 0.25), "medium": int(count * 0.25),
                "hard": int(count * 0.25)}
        dist["evil"] = count - dist["easy"] - dist["medium"] - dist["hard"]
    else:
        q = count // 4
        dist = {"easy": q, "medium": q, "hard": q, "evil": count - 3 * q}

    puzzles = generate_puzzle_set(count, size)
    ...
    for i, p in enumerate(puzzles):
        ...
        solution = p["solution"]
        diff = p["difficulty"]
        diff_num = p["difficulty_num"]
        n_givens = p["n_givens"]

        pt0 = time.time()
        for strategy in STRATEGIES:
            solved, log = solve_with_strategy(puzzle, strategy, size)
            if not is_solved(solved, size):
                solved = solution
            m = compute_metrics(log, solved, size)

            results.append({
                "puzzle_id": i + 1, "size": size,
                "difficulty": diff, "difficulty_num": diff_num,
                "n_givens": n_givens, "strategy": strategy,
                **m
            })

        pt = time.time() - pt0
        el = time.time() - t0
        rate = (i + 1) / el if el > 0 else 0.01
        eta = (len(puzzles) - i - 1) / rate
        sys.stdout.write(f"\r  P{i+1}/{len(puzzles)} {diff:<6} 5strats {pt:.1f}s | {el:.0f}s ETA:{eta:.0f}s   ")
        sys.stdout.flush()

    print(f"\n  Done: {len(results)} rows in {time.time()-t0:.1f}s")
    return results


def run_phase1_analysis(results):
    """Analyze Phase 1: ρ between strategy rank and each sensor."""
    try:
        from scipy.stats import spearmanr
    except ImportError:
        import subprocess
    ...
    strategy_ranks = [STRATEGY_RANK[r["strategy"]] for r in results]
    ...
    print(f"\n  ═══════════════════════════════════════════════════════")
    print(f"  Phase 1 — Per-strategy averages")
    print(f"  ═══════════════════════════════════════════════════════\n")
    print(f"  {'Strategy':<10} {'N':>4} {'Steps':>6} {'BT':>5} {'Guess':>5} {'LZstr':>7} {'ADSR':>6} {'Gini':>6}")
    print(f"  {'─'*10} {'─'*4} {'─'*6} {'─'*5} {'─'*5} {'─'*7} {'─'*6} {'─'*6}")
    ...
    for s in STRATEGIES:
        ...
        if not items:
            continue
        n = len(items)
        avg = lambda k: sum(r.get(k, 0) for r in items) / n
        print(f"  {s:<10} {n:>4} {avg('n_steps'):>6.0f} {avg('n_backtracks'):>5.1f} "
              f"{avg('n_guesses'):>5.1f} {avg('LZ_strategy_raw'):>7.4f} "
              f"{avg('ADSR_bimodality'):>6.3f} {avg('gini_cascade'):>6.3f}")

    # ρ between strategy_rank and each sensor
    sensors = [
        ("LZ_strategy_raw", "LZ_strategy NHGB  ", "primary hypothesis"),
        ("LZ_process",      "LZ_proc (binary)  ", "Phase 0 baseline"),
        ("LZ_multi_4",      "LZ_multi |Σ|=4    ", "multi-alphabet"),
        ("ADSR_bimodality", "ADSR (fixed)       ", "bifurkacija"),
        ("ADSR_selfcal",    "ADSR (self-cal)    ", "P17"),
        ("LZ_transition",   "LZ_transition      ", "Cw stability"),
        ("cascade_weighted","cascade_weighted    ", "depth × delta"),
        ("gini_cascade",    "gini_cascade        ", "inequality"),
        ("entropy_rate",    "entropy_rate        ", "Shannon+LZ"),
        ("density_flow",    "density_flow        ", "smoothness"),
        ("n_backtracks",    "backtracks          ", "kontrola"),
        ("n_steps",         "n_steps             ", "kontrola"),
    ]

    print(f"\n  ═══════════════════════════════════════════════════════")
    print(f"  Phase 1 — ρ(strategy_rank, sensor)")
    print(f"  ═══════════════════════════════════════════════════════")
    print(f"  Higher rank = smarter strategy (random=1...human=5)")
    print(f"  Negative ρ = smarter → lower value (fewer steps, less complex)\n")

    rho_results = {}
    for key, label, note in sensors:
        vals = [r.get(key, 0) for r in results]
        if all(v == vals[0] for v in vals):
            rho, p = 0.0, 1.0
        else:
            rho, p = spearmanr(strategy_ranks, vals)
        rho_results[key] = (rho, p)
        star = "★" if abs(rho) > 0.3 else " "
        print(f"  {star} {label} = {rho:+.4f}  (p={p:.2e})  {note}")

    # Per-difficulty breakdown
    print(f"\n  ─── Per-difficulty ρ(strategy_rank, LZ_strategy_raw) ───\n")
    by_diff = defaultdict(list)
    for r in results:
        by_diff[r["difficulty"]].append(r)
    for d in ["easy", "medium", "hard", "evil"]:
        ...
        if not items:
            continue
        sr = [STRATEGY_RANK[r["strategy"]] for r in items]
        vals = [r.get("LZ_strategy_raw", 0) for r in items]
        if all(v == vals[0] for v in vals):
            rho, p = 0.0, 1.0
        else:
            rho, p = spearmanr(sr, vals)
        print(f"  {d:<8} ρ = {rho:+.4f}  (p={p:.2e})  n={len(items)}")
    ...
    any_strong = any(abs(rho) > 0.5 for _, (rho, _) in rho_results.items())
    if any_strong:
        print(f"  ✓ FOUNDING HYPOTHESIS CONFIRMED on constraint satisfaction!")
        print(f"    Smarter algorithm → more structured process")
        print(f"    Best sensor: {best_key} (ρ = {best_rho:+.4f})")
    elif any(abs(rho) > 0.3 for _, (rho, _) in rho_results.items()):
        print(f"  ~ Partial signal detected")
        print(f"    Best sensor: {best_key} (ρ = {best_rho:+.4f})")
    else:
        print(f"  ✗ No strong signal: strategy quality does not predict process structure")
...
def main():
    ...
    parser.add_argument("--mode", type=str, default="phase0", choices=["phase0", "phase1"],
                        help="phase0 (default) or phase1 (same puzzle × 5 strategies)")
...
══════════════════════════════════════════════════════""")
    ...
    # ── Phase 1 mode: same puzzle × 5 strategies ──
    if args.mode == "phase1":
        print(f"\n  Mode: PHASE 1 — Same puzzle × 5 strategies")
        print(f"  Config: {args.count} puzzles × {len(STRATEGIES)} strategies = {args.count * len(STRATEGIES)} solve logs")
        print(f"  Strategies: {', '.join(STRATEGIES)}")
        print(f"  Seed: {args.seed}\n")

        t0 = time.time()
        results = run_phase1(count=args.count, size=args.size, seed=args.seed)

        # Save CSV
        csv_out = args.csv if args.csv != "sudoku_phase0_results.csv" else "phase1_results.csv"
        with open(csv_out, "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=PHASE1_CSV_FIELDS, extrasaction='ignore')
            w.writeheader()
            for r in results:
                w.writerow(r)
        print(f"\n  CSV: {csv_out}")

        run_phase1_analysis(results)
        print(f"\n  Total: {time.time()-t0:.1f}s")
        print("══════════════════════════════════════════════════════\n")
        return 0

    # ── Phase 0 mode (default): multi-worker launcher ──
