...
test('15-stop search is bounded per step, preserves best, and rejects oversized or invalid matrices',()=>{
  const D=costs(15), search=BF.create(D,5,true);
  ...
  assert.equal(second.total,87178291200);
  assert.throws(()=>BF.create(costs(16),0,true),/2–15/);
...
test('15-stop Cancel returns the latest best and stops; a subsequent heuristic job succeeds',async()=>{
  const D=costs(15), messages=[];let cancelledAt, w;
  const result=await new Promise((resolve,reject)=>{
    ...
    w.onmessage({data:{type:'solve',profile:'brute',jobId:92,points:points(15),distanceMatrix:D,startIdx:7,roundTrip:false}});
  });
  ...
  w.onmessage({data:{type:'solve',profile:'standard',jobId:93,points:points(15),distanceMatrix:D,startIdx:7,roundTrip:false}});
  assert.equal(messages.at(-1).type,'result');assert.equal(messages.at(-1).jobId,93);
  assert.equal(messages.at(-1).exact,false);
});

test('worker rejects 16 stops even if the UI guard is bypassed',()=>{
  const messages=[],w=worker(m=>messages.push(m));
  w.onmessage({data:{type:'solve',profile:'brute',jobId:94,points:points(16),distanceMatrix:costs(16),startIdx:0,roundTrip:true}});
  assert.equal(messages.length,1);assert.equal(messages[0].type,'error');assert.match(messages[0].error,/2–15/);
});
